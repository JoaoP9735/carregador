...
Bhv_GoalieChaseBall::execute( PlayerAgent * agent )
{
    ...
    // post_proximity_threshold = 2.0;
    // if ((wm.self().pos().y != 0.0)) {
    //     dlog. addText (Logger::TEAM,
    //     __FILE__": goalie near the post, moving to center");
    //     Vector2D center_goal = Vector2D(wm.self().pos().x, 12.0);
    //     doGoToCatchPoint(agent, center_goal);
    // }
                                       ...
                                       const Vector2D & target_point )
{
    ...
    const double angle_buf
        = std::fabs( AngleDeg::atan2_deg( SP.catchableArea() * 1.0, rel.r() ) );
...
<<<<<<< HEAD
 ////////////////////////////////////////////////////////////////////////////// 
bool Bhv_GoalieChaseBall::go_to_center(PlayerAgent * agent)
{
    const ServerParam & SP = ServerParam::i();
    const WorldModel & wm = agent->world();

    // Define a posição central do gol:
    Vector2D center_goal_pos(-SP.pitchHalfLength() + 2.0, 0.0);

    // Verifica se a bola está no campo de defesa (x negativo) ou se a distancia é menor que 23 metros para o centro do gol
    if ((wm.ball().pos().dist(center_goal_pos) < 17.0))
    {
        // Se a bola não estiver no campo de defesa ou está mais perto do que 20 metros
        return false;
    }

    // Ajusta a posição alvo baseado na coordenada Y da bola
    if (wm.ball().pos().y > 16.5)
    {
        center_goal_pos.y = SP.goalHalfWidth() / 2;
    }
    else if (wm.ball().pos().y < -16.5)
    {
        center_goal_pos.y = -SP.goalHalfWidth() / 2;
    }

    // Calcula a posição relativa ao centro do gol
    Vector2D rel = center_goal_pos - wm.self().pos();
    rel.rotate(-wm.self().body());
    AngleDeg rel_angle = rel.th();
    const double angle_buf = std::fabs(AngleDeg::atan2_deg(SP.catchableArea() * 0.9, rel.r()));

    dlog.addText(Logger::TEAM,
                 __FILE__": GoToCenter. Center Pos (%.1f, %.1f). angle_diff=%.1f. angle_buf=%.1f",
                 center_goal_pos.x, center_goal_pos.y,
                 rel_angle.degree(), angle_buf);

    agent->debugClient().setTarget(center_goal_pos);

    // Usa doGoToCatchPoint para mover ao centro se não já estiver lá
    if (wm.self().pos().dist(center_goal_pos) > 1.0)
    {
        doGoToCatchPoint(agent, center_goal_pos);
    }
