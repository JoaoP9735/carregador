...
void StrictCheckPassGenerator::evaluateCourses(const WorldModel & world){
    ...
    for (int i=0;i<M_courses.size();i++)
    {
        ...
        if (auto passe = std::dynamic_pointer_cast<Pass>(M_courses[i])) {
        ...
        {
            AngleDeg pass_angle = ( passe->receiver->pos() - world.self().pos() ).th();
            int max_count = 0;
        ...
        double offense_rate
            = std::pow( 0.97,
                        std::max( 5.0, std::fabs( passe->M_target_point.y
                                                  - world.ball().pos().y ) ) );
        //-----------------------------------------------------------
