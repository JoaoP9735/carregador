 ...
 */
double Strategy::get_normal_dash_power(const WorldModel & wm)
{
    ...
    else if (wm.self().stamina() < ServerParam::i().staminaMax() * 0.6) // Ajuste do limiar para entrar em modo de recuperação
    {
        s_recover_mode = true;
    }
    else if (wm.self().stamina() > ServerParam::i().staminaMax() * 0.8) // Ajuste do limiar para sair do modo de recuperação
    {
        s_recover_mode = false;
    }
    ...
    {
        dash_power = my_inc - 45.0; // Redução mais agressiva para recuperação
    ...
    {
        dash_power = std::min(my_inc * 1.0, ServerParam::i().maxDashPower()); // Ajuste do valor baseado no incremento de stamina
    }
    ...
    {
        dash_power = std::min(my_inc * 1.55, ServerParam::i().maxDashPower()); // Redução do multiplicador para conservação de stamina
    }
